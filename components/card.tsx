...
const Card = ({ className, asChild = false, ...props }: React.ComponentProps<"div"> & {asChild?: boolean}) => {
		const Comp = asChild ? Slot : "div"
		return (
			<Comp
				data-slot="card"
				className={cn("bg-card p-4 rounded-lg shadow-md border text-sm",className)}
				{...props}
			/>
		)
	}
